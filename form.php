...
// --- AJAX HANDLER FOR DELETING CLIENT PROFILE ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_client') {
    ...
    // Safely wipe client details without deleting their sales history
    $sql = "UPDATE sales SET tin = NULL, address = NULL, contact_person_contact = NULL, payment_term = NULL WHERE company = '$comp'";
    if ($conn->query($sql)) {
...
// 2. MERGE WITH NEW CSV (COMPANY NAME, COMPANY ADDRESS, TIN, NAM SALES)
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if (!empty($compName)) {
            if (!isset($clientData[$compName])) {
                $clientData[$compName] = ['tin' => $tin, 'address' => $address, 'contact' => '', 'term' => ''];
            } else {
                if (empty($clientData[$compName]['tin']) && !empty($tin)) $clientData[$compName]['tin'] = $tin;
                if (empty($clientData[$compName]['address']) && !empty($address)) $clientData[$compName]['address'] = $address;
            }
        }
    }
    fclose($handle);
...
<body class="bg-light">
        ...
        <div class="modal-dialog">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="fas fa-building me-2"></i>Edit Client Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body bg-light">
                    <form id="clientForm" onsubmit="saveClient(event)">
                        <div class="mb-3">
                            ...
                            <input type="text" id="modalClientName" class="form-control fw-bold text-primary" required>
                        </div>
                        <div class="row g-2 mb-3">
                            <div class="col-6">
                                <label class="form-label fw-bold small text-muted">TIN</label>
                                <input type="text" id="modalClientTIN" class="form-control" placeholder="000-000-000-000">
                            </div>
                            <div class="col-6">
                                <label class="form-label fw-bold small text-muted">Payment Term</label>
                                <input type="text" id="modalClientTerm" class="form-control" placeholder="e.g. 30 Days">
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-bold small text-muted">Address</label>
                                <textarea id="modalClientAddress" class="form-control" rows="2" placeholder="Company Address"></textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label fw-bold small text-muted">Contact Person</label>
                                <input type="text" id="modalClientContact" class="form-control" placeholder="Name / Number">
                            </div>
                        </div>
                        <div class="d-grid">
                            <button type="submit" id="btnSaveClient" class="btn btn-primary fw-bold"><i class="fas fa-save me-2"></i>Save Details</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        // --- MANAGE CLIENT LOGIC (WITH NEW DELETE OPTION) ---
        ...
        function renderClientList(filter = '') {
            ...
            Object.keys(clients).forEach(comp => {
                if(comp.toLowerCase().includes(filter)) {
                    ...
                    tr.innerHTML = `
                        <td class="fw-bold text-primary align-middle">${comp}</td>
                        <td class="align-middle"><small>${c.tin || '-'}</small></td>
                        <td class="align-middle"><small class="d-inline-block text-truncate" style="max-width: 150px;">${c.address || '-'}</small></td>
                        <td class="text-end align-middle text-nowrap">
                            <button class="btn btn-sm btn-outline-warning py-1 px-2 shadow-sm" onclick="editSpecificClient('${safeComp}')" title="Edit Client Details"><i class="fas fa-edit"></i></button>
                            <button class="btn btn-sm btn-outline-danger py-1 px-2 shadow-sm ms-1" onclick="deleteClient('${safeComp}')" title="Wipe Client Profile"><i class="fas fa-trash-alt"></i></button>
                            <button class="btn btn-sm btn-primary py-1 px-2 shadow-sm ms-1" onclick="selectClient('${safeComp}')" title="Select for Sales Form"><i class="fas fa-check"></i></button>
                        </td>
                    `;
        ...
        window.selectClient = function(comp) {
            document.getElementById('companyInput').value = comp;
            fillClientDetails(comp);
            clientListModalInstance.hide();
        };

        window.editSpecificClient = function(comp) {
            clientListModalInstance.hide(); 
            const c = clients[comp] || {};
            
            document.getElementById('modalClientName').value = comp;
            document.getElementById('modalClientTIN').value = c.tin || '';
            document.getElementById('modalClientAddress').value = c.address || '';
            document.getElementById('modalClientContact').value = c.contact || '';
            document.getElementById('modalClientTerm').value = c.term || '';
            
            clientModalInstance.show(); 
        };

        // NEW: Function to safely wipe client details
        window.deleteClient = async function(comp) {
            if(!confirm(`Are you sure you want to delete the profile details for "${comp}"?\n\nNote: This is safe. It will ONLY wipe their TIN, Address, and Contact info. Their past sales history will remain intact.`)) return;
            ...
            try {
                const res = await fetch('form.php', { method: 'POST', body: formData });
                ...
                if(data.success) {
                    ...
                    // If they had this client selected in the form, clear the inputs
                    if(document.getElementById('companyInput').value === comp) {
                        document.querySelector('[name="tin"]').value = '';
                        document.querySelector('[name="address"]').value = '';
                        document.querySelector('[name="contact_person_contact"]').value = '';
                        document.querySelector('[name="payment_term"]').value = '';
                    }
                } else {
        ...
        function openClientModal() {
            const currentInput = document.getElementById('companyInput').value;
            const c = clients[currentInput] || {};
            
            document.getElementById('modalClientName').value = currentInput;
            document.getElementById('modalClientTIN').value = document.querySelector('[name="tin"]').value || c.tin || '';
            document.getElementById('modalClientAddress').value = document.querySelector('[name="address"]').value || c.address || '';
            document.getElementById('modalClientContact').value = document.querySelector('[name="contact_person_contact"]').value || c.contact || '';
            document.getElementById('modalClientTerm').value = document.querySelector('[name="payment_term"]').value || c.term || '';
        ...
        async function saveClient(e) {
            ...
            const term = document.getElementById('modalClientTerm').value;
            ...
            try {
                ...
                if(data.success) {
                    ...
                    document.querySelector('[name="contact_person_contact"]').value = contact;
                    document.querySelector('[name="payment_term"]').value = term;
                    
                    if(!clients[company]) clients[company] = {};
                    clients[company].tin = tin;
                    clients[company].address = address;
                    clients[company].contact = contact;
                    clients[company].term = term;
        ...
        window.submitBatch = async function(destination) {
            ...
            if (destination === 'quotations') {
                ...
                const header = {
                    ...
                    quote_ref: 'RES-' + Date.now().toString().slice(-6),
